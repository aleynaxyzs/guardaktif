...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: "hyrest System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: "hyrest System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  if (member.user.bot) {
  member.guild.roles.cache.forEach(async function(ozi) {
  ...
  member.guild.members.ban(entry.executor.id, {reason: "hyrest System | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: "hyrest System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.roles.cache.forEach(async function(ozi) {
  if (ozi.permissions.has("ADMINISTRATOR") || ozi.permissions.has("BAN_MEMBERS") || ozi.permissions.has("MANAGE_GUILD") || ozi.permissions.has("KICK_MEMBERS") || ozi.permissions.has("MANAGE_ROLES") || ozi.permissions.has("MANAGE_CHANNELS")) {
    ozi.setPermissions(0).catch(err =>{});}});

  newGuild.members.ban(entry.executor.id, {reason: "hyrest System | URL Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `hyrest System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client2.on("roleDelete", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `hyrest System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.delete({ reason: "hyrest System | Rol Koruma Sistemi" });
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  newRole.guild.members.ban(entry.executor.id, { reason: `hyrest System | İzinsiz Rol Güncelleme!` }).catch(e => { })	
...
client2.on("guildMemberUpdate", async (oldMember, newMember) => {
  ...
  newMember.roles.set(oldMember.roles.cache.map(r => r.id));
    uyecik.guild.members.ban(entry.executor.id, { reason: `hyrest System | İzinsiz Yönetici Verme!` }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.roles.cache.forEach(async function(ozi) {
  if(ozi.permissions.has("ADMINISTRATOR") || ozi.permissions.has("BAN_MEMBERS") || ozi.permissions.has("MANAGE_GUILD") || ozi.permissions.has("KICK_MEMBERS") || ozi.permissions.has("MANAGE_ROLES") || ozi.permissions.has("MANAGE_CHANNELS")) {
    ozi.setPermissions(0).catch(err =>{});}});

  channel.guild.members.ban(entry.executor.id, { reason: `hyrest System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "hyrest System | Kanal Korum Sistemi!" }).then(async kanal => {
...
channel2.send(`@here`, {embed: ozi}).catch(e => { })
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `hyrest System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: "hyrest System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.cache.roles.cache.forEach(async function(ozi) {
  if(ozi.permissions.has("ADMINISTRATOR") || ozi.permissions.has("BAN_MEMBERS") || ozi.permissions.has("MANAGE_GUILD") || ozi.permissions.has("KICK_MEMBERS") || ozi.permissions.has("MANAGE_ROLES") || ozi.permissions.has("MANAGE_CHANNELS")) {
    ozi.setPermissions(0).catch(err =>{});}});

  newChannel.guild.members.ban(entry.executor.id, { reason: `hyrest System | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "hyrest System | Webhook Silindi!"}))
  channel.guild.members.ban(entry.executor.id, {reason: "hyrest System | İzinsiz Webhook Açmak!"})
...
.setDescription(`${entry.executor} üyesi izinsiz emoji sildi ve kullanıcıyı karantina atıp, emojiyi yeniden yükledim.\n─────────────────────\nYetkili: (${entry.executor} - \`${entry.executor.id}\`)\nEmoji: \`${emoji.name}\` - \`${emoji.id}\`\n─────────────────────\nTarih: \`${moment(Date.now() + (1000*60*60*3)).format("LLL")}\``)
  channel.send(`@here`, {embed: ozi}).catch(err => { })
return;
});
